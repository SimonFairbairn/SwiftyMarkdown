...
@objc open class SwiftyMarkdown: NSObject {
    ...
    */
    ...
    open func getParsedMarkdownElements(from markdownString : String? = nil) -> [ParsedMarkdownElement] {
        ...
        var result: [ParsedMarkdownElement] = []
        
        for (idx, line) in referencesRemoved.enumerated() {
            if idx > 0 {
                attributedString.append(NSAttributedString(string: "\n"))
            }
            if line.line.hasPrefix(ParsingMakrdownDelimiterItem.image.rawValue) {
                let finalTokens = self.tokeniser.process(line.line)
                self.perfomanceLog.tag(with: "(tokenising complete for line \(idx)")
                let imageElement = ParsedMarkdownElement.image(caption: finalTokens.first?.inputString ?? "", urlString: finalTokens.first?.metadataStrings.first ?? "")
                result.append(.string(attributedString))
                result.append(imageElement)
                attributedString = NSMutableAttributedString(string: "")
            }
            // Append more 'if' cases handling when new delimiter added.
            else {
                let finalTokens = self.tokeniser.process(line.line)
                self.previouslyFoundTokens.append(contentsOf: finalTokens)
                self.perfomanceLog.tag(with: "(tokenising complete for line \(idx)")
